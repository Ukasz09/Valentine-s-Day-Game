...
public abstract class AllLevels {
    ...
    private void spawnCoins(ArrayList<Coin> coinsList, int amountOfSmallCoins, int amountOfNormalCoins, int amountOfBigCoins) {
    ...
    private void renderMonsters(ArrayList<Monster> enemiesList) {
        if (enemiesList != null) {
    ...
    private void renderCoins(ArrayList<Coin> coinsList) {
        if (coinsList != null) {
            for (Coin m : coinsList)
                m.render();
        }
    ...
    private void renderBackground(Image backgroundImage) {
        getManager().getGraphicContext().drawImage(backgroundImage, 0, 0);
    }

    private void renderShots(ArrayList<Weapon> weapons) {
        for (Weapon weapon : weapons) weapon.render();
    }
    ...
    private void renderText(String text, Color color, double posX, double posY) {
        manager.getGraphicContext().setFill(color);
        manager.getGraphicContext().fillText(text, posX, posY);
    }

    private void renderScoreText(int score) {
        String pointsText = MONEY_COMMUNICATE + score;
        renderText(pointsText, Color.TAN, 0, 75);
    }

    protected void setAmountOfCoins(int amountOfSmallCoins, int amountOfNormalCoins, int amountOfBigCoins) {
